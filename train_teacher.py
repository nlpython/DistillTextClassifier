...
from utils.processer import Processer
...
def train():
    logger.add("logs/teacher_train.log")
    ...
    train_loader = DataLoader(
        dataset=ClassifierDataset(processer.convert_examples_to_features('train')),
        batch_size=args.batch_size,
        shuffle=True,
        collate_fn=ClassifierDataset.collate_fn
    )
    valid_loader = DataLoader(
        dataset=ClassifierDataset(processer.convert_examples_to_features('test')),
        batch_size=args.batch_size,
        shuffle=False,
        collate_fn=ClassifierDataset.collate_fn
    )
    ...
    # define model
    model = BertClassifier.from_pretrained(args.bert_path, processer.tokenizer).to(device)
